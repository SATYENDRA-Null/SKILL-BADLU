...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!name || !name.trim()) {
      return res
        .status(400)
        .json({ error: "INVALID_INPUT", message: "Full name is required." });
    }
    ...
    return res.status(201).json({
      success: true,
      message:
        "Application submitted successfully! Your profile is pending Admin Verification.",
      status: "PENDING_VERIFICATION",
  ...
  try {
    ...
    if (user.verification_status === "REJECTED") {
      return res.status(403).json({
        ...
        message:
          ...
          "Your application was not approved during sovereign admin KYC review.",
        user: sanitizeUser(user)
      });
    }
    ...
    if (user.verification_status !== "APPROVED") {
      return res.status(403).json({
        ...
        message:
          "Aapka account admin verification ke liye pending hai. Admin approval milne ke baad aap login kar sakenge."
      });
    }
    ...
    if (user.fee_status !== "PAID") {
      return res.status(402).json({
        ...
        message:
          "Admin ne aapka account approve kar diya hai! Login complete karne ke liye ₹99 onboarding fee pay karein aur 50 Welcome Credits paayein."
      });
    }
  ...
  try {
    ...
    if (user.verification_status === "REJECTED") {
      return res.json({
        ...
        message:
          user.rejection_reason ||
          "Application was not approved by admin review.",
        user: sanitizeUser(user)
      });
    }
    ...
    if (user.fee_status !== "PAID") {
      return res.json({
        ...
        currency: "INR",
        message:
          "Account verified! ₹99 onboarding payment is required to activate login.",
        user: sanitizeUser(user)
      });
    }
