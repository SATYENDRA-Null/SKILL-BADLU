...
class MatchmakingEngine {
  constructor(store) {
    this.store = store;
    this.weights = {
      w1: 0.30, // Skill Tag Overlap
      w2: 0.15, // Level Compatibility
      w3: 0.35, // Mutual Swap Bonus (Reciprocal Swap)
      w4: 0.10, // Historical Peer Rating
      w5: 0.10  // Activity Recency
    };
  }
  ...
  scoreCandidate(candidate, targetUser) {
    ...
    const overlapScore = union.size > 0 ? intersection.length / union.size : 0.0;

    // 2. Level Compatibility
    // Check if candidate skill level meets or exceeds user desired level
    let levelScores = [];
    intersection.forEach(skillId => {
      const candSkill = candidate.skills_have.find(s => s.skill_id === skillId);
      const userSkill = targetUser.skills_want.find(s => s.skill_id === skillId);
      if (candSkill && userSkill) {
        levelScores.push(candSkill.level >= userSkill.level ? 1.0 : 0.5);
      }
    });
    const levelCompat = levelScores.length > 0 
    ...
    const ratingScore = Math.max(0.0, Math.min(1.0, (candidate.avg_rating - 1.0) / 4.0));

    // 5. Recency Bonus: exp(-days_inactive / 14)
    const deltaDays = Math.max(0.0, (Date.now() - new Date(candidate.last_active).getTime()) / (1000 * 60 * 60 * 24));
    ...
    return {
      ...
      matchingSkills: {
        candTeaches: candidate.skills_have.filter(s => userWantIds.has(s.skill_id)),
        userTeaches: targetUser.skills_have.filter(s => candWantIds.has(s.skill_id))
      }
    };
  }
