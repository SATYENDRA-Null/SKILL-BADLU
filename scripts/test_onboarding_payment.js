...
const ledger = window.ledger = new (require("../js/ledger.js").LedgerEngine || class {
  constructor(st) { this.store = st; }
  getBalance(userId) {
    let creditsIn = 0, creditsOut = 0;
...
const admin = window.admin = new (class {
  constructor(st, ld) { this.store = st; this.ledger = ld; }
  getPendingUsers() { return this.store.users.filter(u => u.verified_status === "PENDING_REVIEW"); }
  approveUser(userId) {
...
if (initialMetrics.totalRevenue < 396) throw new Error("Initial revenue should be at least ₹396");
if (initialMetrics.paidCount < 4) throw new Error("Initial paid count should be at least 4");
...
const newApplicant = store.registerUser({
  ...
  skillsHave: [{ skill_id: "sk_python", name: "Python", category: "tech", level: 3 }]
});

console.log(`Registered: ${newApplicant.name} (${newApplicant.id})`);
console.log(`Status: ${newApplicant.verified_status}, Fee: ${newApplicant.fee_status}`);
...
console.log("✓ Pre-login Admin Verification gate blocked unverified access as expected.");
...
console.log(`Approved Deepak Verma. Current status: ${newApplicant.verified_status}, Fee Status: ${newApplicant.fee_status}`);
if (newApplicant.verified_status !== "VERIFIED" || newApplicant.fee_status !== "PENDING_PAYMENT") {
  throw new Error("Applicant should be VERIFIED and PENDING_PAYMENT after admin approval.");
}
...
if (postVerifCheck.eligible !== false || postVerifCheck.reason !== "PAYMENT_REQUIRED") {
  throw new Error("Verified applicant with unpaid fee must trigger PAYMENT_REQUIRED.");
}
...
console.log(`Deepak Verma Fee Status: ${newApplicant.fee_status}, Amount Paid: ₹${newApplicant.fee_amount}`);
