...
console.log(
  "Approved courses:",
  store.getApprovedCourses().map((c) => c.title)
);
console.log(
  "Pending review courses:",
  store.getPendingCourses().map((c) => c.title)
);
console.assert(
  store.courses.length >= 5,
  "Should have at least 5 initial courses"
);
console.assert(
  store.getPendingCourses().length >= 1,
  "Should have 1 pending course initially"
);
...
console.assert(
  newBal === initialBal - 35,
  "Balance should be reduced by 35 CR"
);
...
console.assert(
  ...
  "Enrollment should initially be uncompleted"
);

console.log("\n=== 3. TESTING ANTI-SKIP INTERVAL MERGING & DETECTION ===");
// Test Interval merging
const merged = education.mergeIntervals([
  [0, 10],
  [5, 15],
  [20, 30]
]);
console.log(
  "Merged intervals [[0,10],[5,15],[20,30]]:",
  JSON.stringify(merged)
);
console.assert(
  merged.length === 2 && merged[0][0] === 0 && merged[0][1] === 15,
  "Intervals should merge correctly"
);
...
console.log(
  "Skip detection triggered when seeking from 5s to 35s:",
  education.playbackTracker.forwardSkipOccurred
);
console.assert(
  education.playbackTracker.forwardSkipOccurred === true,
  "Forward skip should be detected!"
);
...
console.log(
  `Continuous playback watched: ${watchedSec}s / 50s (${coverage * 100}%)`
);
console.assert(
  coverage >= 0.95 && !education.playbackTracker.forwardSkipOccurred,
  "Should pass anti-skip coverage"
);
...
console.assert(
  cert.id.startsWith("CERT-SB-"),
  "Certificate ID format should start with CERT-SB-"
);
console.assert(
  store.getUserCertificates("user_a").length >= 1,
...
const newCourse = store.submitCourse({
  ...
  description:
    "Deep dive into Next.js 15 streaming, caching, and server actions."
});

console.log("Newly submitted course status:", newCourse.status);
console.assert(
  newCourse.status === "PENDING_REVIEW",
  "New course must start in PENDING_REVIEW"
);
...
console.assert(
  foundInQueue !== undefined,
  "Course must be visible in admin queue"
);
...
console.assert(
  ...
  "Course status must be APPROVED"
);
...
console.assert(
  isLiveInCatalog === true,
  "Course must appear in public catalog"
);
...
console.log(
  "Kaito Tanaka upload permission after admin grant:",
  userD.can_upload_videos
);
console.assert(
  userD.can_upload_videos === true,
  "User D should have video upload permission granted"
);

admin.revokeCreatorPermission("user_d");
console.log(
  "Kaito Tanaka upload permission after admin revoke:",
  userD.can_upload_videos
);
console.assert(
  userD.can_upload_videos === false,
  "User D upload permission should be revoked"
);
